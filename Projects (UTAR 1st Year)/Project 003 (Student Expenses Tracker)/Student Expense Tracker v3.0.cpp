...
int main() {
    ...
    do {
        ...
        cout << "5. Exits.\n";
        choice = validInt("Enter choice: ", 1, 5);

        switch (choice) {
            case 1: {
                double household, part;
                household = validDouble("Enter household expenses: ", 0.00);
                bud.setHousehold(household);
                part = validDouble("Enter part-time salary: ", 0.00);
                bud.setSalary(part);
                bud.saveBudget();
                break;
            }
